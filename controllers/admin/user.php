...
if (isset($_GET['user'])) {
    if ($_GET['user'] == 'update') {
        ...
        header('location: /tmaster/pages/secure/admin/user.php' . $params);
    }
...
function updateProfile($req)
{
    ...
    if (isset($data['invalid'])) {
        $_SESSION['errors'] = $data['invalid'];
        $params = '?' . http_build_query($req);
        header('location: /tmaster/pages/secure/user/profile.php' . $params);
        } else {
        $user = user(); 
        $data['id'] = $user['id'];
...
function changePassword($req)
{
    ...
    if (isset($data['invalid'])) {
        $_SESSION['errors'] = $data['invalid'];
        $params = '?' . http_build_query($req);
        header('location: /tmaster/pages/secure/user/password.php' . $params);
    } else {
        $data['id'] = userId();
        $success = updatePassword($data);
        if ($success) {
            $_SESSION['success'] = 'Password successfully changed!';
            header('location: /tmaster/pages/secure//user/password.php');
        }
    }
}
